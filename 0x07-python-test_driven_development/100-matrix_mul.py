...
def matrix_mul(m_a, m_b):
    ...
    Raises:
        ...
        TyprError: If one element of list of lists is not int/float
        TypeError: If row of m_a or m_b are not the same size
        ValueEror: If m_a or m_b is empty
        ValueError: If m_a or m_b cannot be multiplied

    Returns:
        Anew list which is the outcome of the multiplication
    ...
    if not all(len(row) == len(n_a[0]) for row in m_a):
